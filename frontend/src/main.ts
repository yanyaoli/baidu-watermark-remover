/**
 * 图片去水印 V2 — 前端逻辑（TypeScript + Tailwind）
 * 职责：图片收集与压缩、遮罩涂抹（全分辨率导出）、逐张提交与轮询、结果展示。
 * 遮罩规范化在浏览器完成，服务端无状态。
 */
import "./style.css";
...
function renderGallery() {
  ...
  for (const img of images) {
    ...
    card.className =
      "relative aspect-[4/3] overflow-hidden rounded-xs border border-line bg-panel transition-colors hover:border-line-strong";
    ...
    name.className =
      "text-shadow-thumb pointer-events-none absolute inset-x-2 top-2 truncate text-[11px] text-white/85";
    ...
    if (img.mask) {
      ...
      badge.className =
        "absolute left-2 top-2 flex items-center gap-1 rounded-xs border border-line-strong bg-[#0e0e0e]/80 px-2 py-0.5 text-[11px] backdrop-blur-sm";
    ...
    actions.className =
      "absolute inset-x-0 bottom-0 flex justify-end gap-1.5 bg-gradient-to-t from-black/75 to-transparent p-2 pt-6 opacity-0 transition-opacity duration-200 group-hover:opacity-100";
  ...
  for (const img of doneImages) {
    ...
    `;
      card.querySelector('[data-act="dl"]')!.addEventListener("click", async () => {
        // 直接从返回的图片地址下载：fetch 转 Blob 触发保存；跨域受限时新标签页打开
        const url = img.resultUrl!;
        const name = `去水印_${idx}.jpg`;
        try {
...
function updateActionbar() {
  ...
  $("actionbarInfo").textContent = running
    ? `处理中… ${images.filter((i) => i.status === "done").length}/${images.length} 完成`
    : `已选择 ${images.length} 张`;
...
async function processImages(targets: AppImage[]) {
  ...
  await Promise.all(
    Array.from({ length: Math.min(SUBMIT_CONCURRENCY, targets.length) }, worker)
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // logo 图标：simple-icons 官方路径 + 渐变填充（defs 渐变定义保留在 HTML 中）
  ...
  // GitHub 品牌图标路径来自 simple-icons 官方库
